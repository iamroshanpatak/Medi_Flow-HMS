      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="ml-0 md:ml-64 pt-20 p-4 md:p-8">
          <div className="max-w-7xl mx-auto">
            {/* Header */}
            <div className="mb-8">
              <h1 className="text-4xl font-bold text-gray-900 mb-2">{t('nav.appointments')}</h1>
                  ...
                  <div key={apt._id} className="bg-white rounded-lg shadow-md hover:shadow-lg transition p-6">
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-gray-900 font-semibold">{apt.reason}</p>
                        <p className="text-xs text-gray-600 mt-2">Type: {apt.type}</p>
                      </div>

                      {/* Status */}
                      <div className="flex flex-col items-end justify-between">
                        <div className={`flex items-center gap-2 px-3 py-1 rounded-full text-sm font-semibold ${getStatusColor(apt.status)}`}>
                          {getStatusIcon(apt.status)}
                          <span className="capitalize">{apt.status.replace('-', ' ')}</span>
                        </div>
                        {apt.notes && (
                          <p className="text-xs text-gray-600 mt-2 text-right line-clamp-2">{apt.notes}</p>
                        )}
                      </div>
                    </div>
                  </div>
